...
export const getProducts = async (req: Request, res: Response, next: NextFunction): Promise<any> => {
  try {
    ...
    if (minPrice || maxPrice) {
      where.price = {};
      if (minPrice) where.price[Op.gte] = parseFloat(String(minPrice));
      if (maxPrice) where.price[Op.lte] = parseFloat(String(maxPrice));
    }
    ...
    const brandInclude: any = { model: Brand, as: 'brand' };
    if (brand) {
      brandInclude.where = { [Op.or]: [{ id: brand }, { slug: brand }] };
    }

    // Sort mapping
    let order: any = [['isFeatured', 'DESC'], ['createdAt', 'DESC']];
    switch (sort) {
      case 'price_asc':
        order = [['price', 'ASC']];
        break;
...
export const createProduct = async (req: Request, res: Response, next: NextFunction): Promise<any> => {
  ...
  try {
    ...
    if (specifications && specifications.length > 0) {
      const specRecords = specifications.map((spec: any) => ({
        productId: product.id,

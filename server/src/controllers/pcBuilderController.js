const { Product, Category, ProductSpecification, ProductImage } = require('../models');
...
const getBuilderComponents = async (req, res, next) => {
  try {
...
const validateBuildCompatibility = async (req, res, next) => {
  try {
    ...
    let cpuSocket = null;
    let moboSocket = null;
    let moboRamType = null;
    let ramType = null;
    let psuWattage = 0;

    if (selections.cpu && selections.cpu.specifications) {
      const socketSpec = selections.cpu.specifications.find(s => s.specKey.toLowerCase().includes('socket'));
      if (socketSpec) cpuSocket = socketSpec.specValue.trim();

      const wattageSpec = selections.cpu.specifications.find(s => s.specKey.toLowerCase().includes('tdp') || s.specKey.toLowerCase().includes('watt'));
    ...
    if (selections.motherboard && selections.motherboard.specifications) {
      const socketSpec = selections.motherboard.specifications.find(s => s.specKey.toLowerCase().includes('socket'));
      if (socketSpec) moboSocket = socketSpec.specValue.trim();

      const ramSpec = selections.motherboard.specifications.find(s => s.specKey.toLowerCase().includes('memory') || s.specKey.toLowerCase().includes('ram'));
    ...
    if (selections.gpu && selections.gpu.specifications) {
      const powerSpec = selections.gpu.specifications.find(s => s.specKey.toLowerCase().includes('power') || s.specKey.toLowerCase().includes('tdp'));
    ...
    if (selections.ram && selections.ram.specifications) {
      const ramTypeSpec = selections.ram.specifications.find(s => s.specKey.toLowerCase().includes('type') || s.specKey.toLowerCase().includes('memory'));
    ...
    if (selections.psu && selections.psu.specifications) {
      const psuSpec = selections.psu.specifications.find(s => s.specKey.toLowerCase().includes('watt'));

const { RepairRequest, RepairQuote, RepairQuoteItem, RepairStatusHistory, TechnicianAssignment, User, sequelize } = require('../models');
const { sendSuccess, sendError } = require('../utils/responseHandler');

const getAssignedRepairs = async (req, res, next) => {
  try {
...
const createOrUpdateQuote = async (req, res, next) => {
  const transaction = await sequelize.transaction();
  try {
    const { repairId } = req.params;
    const {
